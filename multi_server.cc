...
void save_state_unlocked() {
    ...
    for (const auto& pair : rooms) {
        ...
        out << "ROOM|" << room.name << "|" << room_type_to_string(room.type) << "|"
            << room.owner << "|" << room.password_hash << "|" << join_set(room.admins)
...
void broadcast_for_message(const MessageRecord& record, const std::string& text) {
    if (record.room_type == ROOM_PRIVATE) {
        ...
        broadcast_room(record.room, text);
    }
}
...
bool send_chat_text(Client* client, const std::string& text) {
    ...
    shown_name = display_name_unlocked(room, nickname);
    if (!allowed) {
        pthread_mutex_unlock(&state_mutex);
    ...
    std::ostringstream oss;
    oss << "[" << room_name << "] " << shown_name << ": " << text << " {" << current_time_text() << "}\n";
    if (room_type == ROOM_PRIVATE) {
        broadcast_private_room(room_name, owner, oss.str());
    } else {
        broadcast_room(room_name, oss.str());
    }
    return true;
}
...
void handle_permission_command(Client* client, const std::string& command, const std::string& target_nick) {
    ...
    if (command == "/permit") {
        ...
        std::string room_name = client->room;
        if (target_client == NULL || target_client->room != room_name) {
            pthread_mutex_unlock(&state_mutex);
            send_to_client(client, format_notice_line(target + "님은 현재 방에 없습니다."));
            return;
        }
        target_client->room = "lobby";
        int target_sock = target_client->sock;
        pthread_mutex_unlock(&state_mutex);
        send_all(target_sock, format_notice_line("강퇴되어 lobby로 이동했습니다."));
        broadcast_room(room_name, format_notice_line(target + "님이 " + client->nickname + "님에 의해 강퇴되었습니다."));
...
void handle_template_command(Client* client, const std::string& arg) {
    ...
    if (subcommand == "add") {
        ...
        for (const auto& pair : user_templates) {
            if (pair.first.rfind(prefix, 0) == 0) oss << pair.first << " = " << pair.second << "\n";
        }
        send_to_client(client, oss.str());
...
void handle_react(Client* client, const std::string& arg) {
    ...
    if (id_text.empty() || emoji.empty()) {
        send_to_client(client, format_notice_line("사용법: /react <메시지ID> <이모지>"));
        return;
    }
...
void send_help(Client* client) {
    send_to_client(client,
        ...
        "/permit <nick>\n/revoke <nick>\n/delegate <nick>\n/kick <nick>\n/roominfo\n"
        "/template add <key> <message>\n/template del <key>\n/template list\n"
        "/template suggest <prefix>\n/template use <key>\n/react <message_id> <emoji>\n"
...
void handle_command(Client* client, const std::string& line) {
    ...
    if (command == "/help") {
        ...
        if (target_client == NULL) {
            oss << "[profile] " << target << " not found\n";
        } else {
            oss << "[profile] " << target_client->nickname
                << " profile=" << visible_profile_for(target_client)
                << " status=" << visible_status_for(target_client) << "\n";
        }
        pthread_mutex_unlock(&state_mutex);
        send_to_client(client, oss.str());
        ...
        if (room.empty() || password.empty()) send_to_client(client, format_notice_line("사용법: /create_secure <방이름> <비밀번호>"));
        else create_room_command(client, room, ROOM_SECURE, password);
    } else if (command == "/join") {
        handle_join(client, arg);
    } else if (command == "/join_secure") {
        std::string password;
        std::string room = first_token(arg, &password);
        if (room.empty() || password.empty()) send_to_client(client, format_notice_line("사용법: /join_secure <방이름> <비밀번호>"));
        ...
        if (current_room == "lobby") {
            send_to_client(client, format_notice_line("이미 lobby에 있습니다."));
        } else {
...
int main(int argc, char* argv[]) {
    ...
    printf("Chat server v3 started on port %s\n", argv[1]);
